...
class MLPModel(nn.Module):
    def __init__(self, no_num=1000, len_no=10, no_label=10, emb_dim=1, *args, **kwargs):
...
def MLPtrain(model, tr_dataloader, val_dataloader, epochs, l_rate, patience=10, weight_decay=1e-4): 
    model.to(device)
    ...
    while epoch<epochs: # Gradient descent over epochs
        ...
        if val_loss<best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), '.\\Saved Models\\mlp_model.pth')
...
def MLPtest(model, test_dataloader):
    model.eval()
    loss_fn = nn.CrossEntropyLoss()
    s_error = 0
    s_matches = 0
...
mlp = MLPModel(emb_dim=2)

# Training the model
loss_history, val_loss_history = MLPtrain(mlp, training_data_loader,validation_data_loader, epochs=25, l_rate=3e-5, weight_decay=1e-3)
print("\nTraining complete! Fetching best model paramaters...")

# Loading best set of model weights
mlp.load_state_dict(torch.load('.\\Saved Models\\mlp_model.pth'))
...
# Testing model
print("\nTesting model!")
MLPtest(mlp, testing_data_loader)
print("Testing done.")