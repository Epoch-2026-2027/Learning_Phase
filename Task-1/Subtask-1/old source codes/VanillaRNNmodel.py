...
class RNNModel(nn.Module):
    def __init__(self, no_num=1000, len_no=10, no_label=10, emb_dim=1, num_layers=1, *args, **kwargs):
        ...
        self.rnn_seq = nn.RNN(input_size=len_no, hidden_size=128, num_layers=num_layers, batch_first=True)
        self.fc = nn.Linear(128, 10*no_label)
...
def RNNtrain(model, tr_dataloader, val_dataloader, epochs, l_rate, patience=10, weight_decay=1e-4): 
...
def RNNtest(model, test_dataloader):
    model.eval()
    loss_fn = nn.CrossEntropyLoss()
    s_error = 0
    s_matches = 0
...
mlp = RNNModel(emb_dim=3, num_layers=2)

# Training the model
loss_history, val_loss_history = RNNtrain(mlp, training_data_loader,validation_data_loader, epochs=50, l_rate=3e-5, weight_decay=1e-3)
print("\nTraining complete! Fetching best model paramaters...")

# Loading best set of model weights
mlp.load_state_dict(torch.load('.\\Saved Models\\rnn_model.pth'))
...
# Testing model
print("\nTesting model!")
RNNtest(mlp, testing_data_loader)
print("Testing done.")
