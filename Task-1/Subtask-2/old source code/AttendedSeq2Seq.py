...
def RNNtrain(model, tr_dataloader, val_dataloader, epochs, l_rate, vocab_size, patience=10, weight_decay=1e-4): 
    ...
    while epoch<epochs: # Gradient descent over epochs
        ...
        if val_loss<best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), '.\\Saved Models\\attended_rnn_model.pth')
...
train_loader = DataLoader(CodeSet(train_data, tokenizer, sample_data=None), batch_size=256, shuffle=True, num_workers=0)
val_loader = DataLoader(CodeSet(val_data, tokenizer, sample_data=None), batch_size=256, shuffle=True, num_workers=0)
test_loader = DataLoader(CodeSet(test_data, tokenizer, sample_data=None), batch_size=256, shuffle=True, num_workers=0)
print(" Done.\n")
# Initialisation
...
model.load_state_dict(torch.load('.\\Saved Models\\attended_rnn_model.pth'))
print(" Done.\n")
