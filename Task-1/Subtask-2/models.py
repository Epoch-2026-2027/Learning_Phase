...
# Tokenizers 
def CharTokenizer(data, max_seq_len, sample_data, purp='src'): # Character Tokenizer deprecated in favour of Word Tokenizer (Code TOkenizer)
    if purp=='src':
...
class CodeTokenizer():
    ...
    def tokenize_single(self, string):
        str_data = [i for i in re.split(r'([. ])', string) if i!=' ']
        leng = len(str_data) if len(str_data)<(self.max_seq_len-1) else self.max_seq_len-1
        tokenized = [(self.tokenizing_func(i) if self.tokenizing_func(i)!=None else self.unk) for i in str_data[:leng] if i!=' '] + [self.eos] + (self.max_seq_len-leng-1)*[self.pad_id]
        return tokenized

    def detokenize_single(self, tok_str):
        string = ""
        for t in range(len(tok_str)):
            if tok_str[min(t+1, len(tok_str)-1)] in (self.eos, self.pad_id, self.del1) or tok_str[t]==self.del1:
                string += self.detokenizing_func(tok_str[t])
            else:
                string += self.detokenizing_func(tok_str[t]) + ' '
        string = re.sub('<EOS>|<PAD>', '', string).rstrip()
        return string

# Dataset
# Transformer version — returns buggy, fixed_infer, fixed_train (fixed_infer has a <BOS> token, which shifts the sequence to the right
# in accordance to the "Attention Is All You Need" paper...)
...
class VanillaSeq2Seq(nn.Module):
    def __init__(self, max_seq_len, vocab_size, emb_dim, hidden_size, pad_id, tf_ratio=0.5, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def forward(self, buggy, fixed=None, tf_ratio=0.5):
        ...
        for t in range(self.max_seq_len):
            ...
            if fixed is not None and torch.rand(1).item()<self.tf_ratio:
                input = fixed[:,t]
            else:
                input = top1
...
#  Attended Seq2Seq (biLSTM and LSTM + Attention)
...
class AttendedSeq2Seq(nn.Module):
    ...
    def forward(self, buggy, fixed=None):
        ...
        hidden = enc_hiddens[:,-1,:].unsqueeze(-2) # Fetching the final encoder hidden state
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, decoder_input, encoder_output):
        x1, att1 = self.maskedmultiheadatt(decoder_input)
        # print(x1, x1.shape)
        # raise Exception("test")
        x1 = self.norm1(x1+decoder_input)
        x2, att2 = self.multiheadcrossatt(x1, encoder_output)
        x2 = self.norm2(x2+x1)
        x3 = self.ffnn(x2)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, decoder_input, encoder_output, seq_len=None):
        ...
        for layer in self.layers:
            x, att1, att2 = layer(x, encoder_output)
            attn1.append(att1)
            attn2.append(att2)
...
class Transformer(nn.Module):
    def __init__(self, N, h, dmodel, dk, dv, max_seq_len, vocab_size, bos, pad_id, viz=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.viz = viz
        self.bos = bos
        self.max_seq_len = max_seq_len
    ...
    def forward(self, buggy, fixed=None):
        encoder_output, enc_att = self.encoder(buggy)
        if fixed is not None:
            x, dec_att, cross_att = self.decoder(fixed, encoder_output)
            x = self.fc1(x)
            out = self.fc2(x)
            return (out, enc_att, dec_att, cross_att) if self.viz else out
        else:
            dummy = (torch.ones((encoder_output.shape[0],1), dtype=torch.int)*self.bos).to(device)
            for _ in range(self.max_seq_len): # Autoregressively generating the output
                x, dec_att, cross_att = self.decoder(dummy, encoder_output, seq_len=dummy.size(-1))
                x = self.fc1(x)
                x = self.fc2(x)
                dummy = torch.cat((dummy, x.argmax(-1)[:,-1].unsqueeze(-1)), dim=-1)
            return (x, enc_att, dec_att, cross_att) if self.viz else x