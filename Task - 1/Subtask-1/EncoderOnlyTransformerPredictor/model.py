...
class Attention(nn.Module):
    def __init__(self,size,size2):
        ...
        self.W_Q = nn.Linear(size,size2)
        self.W_K = nn.Linear(size,size2)
        self.W_V = nn.Linear(size,size2)
        self.sf = nn.Softmax(dim=-1)
        self.size2 = size2

    def forward(self,x):
        return self.sf(1/np.sqrt(self.size2)*((self.W_Q(x))@(self.W_V(x)).mT))@self.W_V(x)
